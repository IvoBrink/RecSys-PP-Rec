...
def trans2tsp(date_str):
    return int(datetime.strptime(date_str, '%m/%d/%Y %I:%M:%S %p').timestamp())
...
class UserContent:
    ...
    def load_session(self):
        ...
        for line in lines:
            if self.flag == 2:
                _, _, click, imp = line.strip().split('\t')
            elif self.flag == 1:
                _, click, imp = line.strip().split('\t')
